---
title: "TAC histórico del boquerón"
...
```{r fig.width=8, fig.height=5}
# Crear el gráfico con ambas medias (global y 2018–2024) y sus etiquetas
ggplot(catchpropSem, aes(x = year, y = proportion, color = as.factor(semester), group = semester)) +
  geom_line(size = 1.1) +
  geom_point() +
  
  # Medias globales (líneas y texto)
  geom_hline(data = media_semestres,
             aes(yintercept = media, color = as.factor(semester)),
             linetype = "dashed", size = 0.8, show.legend = FALSE) +
  geom_text(data = media_semestres,
            aes(x = max(catchpropSem$year) - 35, y = media,
                label = paste0("media histórica = ", round(media, 2)),
                color = as.factor(semester)),
            hjust = 0, vjust = 1, size = 3.5, show.legend = FALSE) +
  
  # Medias 2018–2024 (líneas y texto)
  geom_hline(data = media_semestres_2018_2024,
             aes(yintercept = media, color = as.factor(semester)),
             linetype = "dotted", size = 0.8, show.legend = FALSE) +
  geom_text(data = media_semestres_2018_2024,
            aes(x = max(catchpropSem$year) - 6, y = media,
                label = paste0("media 2018–2024 = ", round(media, 2)),
                color = as.factor(semester)),
            hjust = 0, vjust = 1.5, size = 3.5, show.legend = FALSE) +
  
  # Línea vertical en 2018
  geom_vline(xintercept = 2018, linetype = "solid") +
